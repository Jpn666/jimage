	...
	if (jpgr->isinterleaved == 0) {
		...
		for (y = 0; y < c->nrows; y++) {
			for (x = 0; x < c->ncols; x++) {
				...
				if (CTB_EXPECT0(decodeblock(jpgr, c, block) == 0)) {
					return 0;
				}
			}
		}
	...
	if (jpgr->ncomponents == 1) {
		...
		for (y = 0; y < c1->nrows; y++) {
			for (x = 0; x < c1->ncols; x++) {
				...
				if (CTB_EXPECT0(decodeblock(jpgr, c1, c1->units[0]) == 0)) {
					return 0;
				}
	...
	if (jpgr->issubsampled == 0) {
		...
		for (y = 0; y < jpgr->nrows; y++) {
			for (x = 0; x < jpgr->ncols; x++) {
				...
				if (CTB_EXPECT0(decodeblock(jpgr, c2, c2->units[0]) == 0)) {
					return 0;
				}
...
readfirstDC(struct TJPGRPrvt* jpgr)
{
	...
	if (jpgr->nscancomponents == 1) {
		...
		for (y = 0; y < totaly; y++) {
			...
			if (CTB_EXPECT0(overread(jpgr)) == 1) {
				return 0;
			}
		}
		return 1;
